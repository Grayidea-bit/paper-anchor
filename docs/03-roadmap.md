...
- [x] [sonnet] T-EM-02 **reembed 維護動作**：`services/reembed.py`（逐文獻 `get_chunks`→`embed_passages`→`update_chunk_embeddings`）+ `routers/maintenance.py`（`POST /api/maintenance/reembed`）+ 沿用 backup `try_begin` 鎖（三方互斥）+ `operation:"reembed"` 進度；測試（互斥／進度／失敗續跑）。依賴：T-EM-01
- [x] [opus] T-BK2-01 **備份匯出 v2**：`repo.dump_chunks`（`embedding::text`）+ `chunks/{uuid}.json` 每文獻一檔 + base64 float32 LE codec + manifest v2（`format_version=2`/`counts.chunks`/`chunk_files`/取 `effective_embed_config`）+ 全量覆蓋；測試（roundtrip/snapshot，向量不進 `dump_table_rows` 白名單）。依賴：T-EM-01
