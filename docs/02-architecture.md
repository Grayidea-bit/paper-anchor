...
| Embedding | NIM retrieval 模型或內建本地模型（fastembed ONNX，1024 維），`llm.py` 內分派 | 與 chat 同供應商即用 NIM；純 Claude 訂閱情境（無 embed key）改走本地模型，Anthropic 無 embedding API（M14，見 D12） |
...
- **Embedding 需帶 `input_type` 參數**（OpenAI SDK 用 `extra_body`）：入庫文件用 `"passage"`，查詢問題用 `"query"`——llm.py 的 embed 介面要區分這兩種模式，用錯會顯著拉低檢索品質。**本地路徑（M14，見 D12）同樣區分 passage/query**（部分模型如 arctic 需在 query 加 `"query: "` 前綴）：用錯不報錯但檢索品質默默劣化，同款陷阱。
...
- **建立優先序（T-TR-04 / T-TR-06）**：
  ...
  2. **從對話萃取（主路徑）**：POST 帶 `source_text`（對話「翻譯」動作的詳細翻譯全文，max 8000 字）但無 `translation` 時，`services/glossary.py` 套用 `app/prompts/glossary_extract.md`（帶術語、目標語言、`source_text`）→ 呼叫 `agent.chat_once()`（走 `chat_backend` 分派，M14 後）→ 解析固定格式「譯文：/註解：」兩行 → `translation` + `notes` 入庫。解析失敗（LLM 未照格式回覆）降級：整段 `strip()` 當 `translation`，`notes` 存空字串。
...
- **manifest 結構**：`{format_version, created_at, app_version, embed_model, embed_dim, counts{documents, projects, annotations, glossary_entries, conversations, messages, pdfs}, pdfs: [{name, document_id, size}]}`。上傳順序：先 PDF、再 `db/`、**最後 manifest**——任一步失敗即中止本輪且不上傳 manifest。保證限縮如下：manifest 存在＝一次完整備份完成的標記；`db/` dumps 為全量覆蓋（`files.update` 同一 file id，中途失敗時遠端可能是「新 db + 舊 manifest」，**不保證與 manifest counts 逐筆一致**）；`pdfs/` 為 append-only，舊 manifest 的 `pdfs` 清單永遠是遠端現存檔案的子集，故**依 manifest 還原永遠一致**。
  > **v2 變更（M14，見 D12）**：`format_version=2`；`embed_model`／`embed_dim` 改取 `llm.effective_embed_config()`（反映實際生效來源而非 `.env`）；`counts` 加 `chunks`；新增頂層 `chunk_files`（每文獻一個 `chunks/{uuid}.json` 的清單）——chunks＋向量（base64 float32）隨備份出庫，還原可免重嵌免重解析。v1 佈局（無 `chunks/`）維持不變、仍可還原。
...
| `embed_source` | embedding 來源選擇 `auto`\|`nim`\|`local`（M14，見 D12；來源選擇是使用者層決策，同 `chat_backend` 先例。**僅來源執行期化**——embed 端點/key/模型維持 `.env`） | 否 |

> `gdrive_client_id`／`gdrive_client_secret`／`backup_interval_hours`／`embed_source` 須同步加入 `routers/settings.py` 的 `SettingsUpdate`（漏加會 PUT 靜默丟棄，見 M11 發現事項）；`gdrive_refresh_token`／`backup_last_run`／`restore_last_run` 不開放 PUT，列入守護測試 WRITE_EXEMPT。
...
### D12 本地 Embedding + digest 分派 + 備份格式 v2（M14）

> **背景**：即使對話切到 Claude 訂閱後端，仍有三處寫死依賴 NIM——ingest 的 chunk embedding、每次提問的 query embedding（rag 檢索）、digest 導讀生成（`digest.py` 直呼 `llm.chat`，繞過 `chat_backend` 分派）。Anthropic 無 embedding API，故補法是**內建本地 embedding 模型**；順帶讓 **embedding 向量進雲端備份**（格式 v2），還原免重嵌免重解析、標註↔chunk 關聯不再丟失。M14 規劃於 M15 前，使用者決定先地基（M15）後執行本里程碑。三個子項對應下述 A／B／C。

#### A. 本地 embedding 與 `embed_source`

- **`embed_source` 三值語意**（settings 鍵，執行期化，見 D10 表）：
  - `auto`（預設）：**有 `embed_api_key` 就用 NIM，否則落本地**——純 Claude 訂閱情境（無 embed key）自動走本地模型，零手動切換。
  - `nim`：強制 NIM（無 key 則明確報錯，不默默改用本地污染向量）。
  - `local`：強制本地模型，忽略 NIM 設定。
- **`embed_passages`／`embed_query` 維持 `llm.py` 唯一入口**，內部依 `embed_source` 分派——兩個既有呼叫點（`ingest.py`、`conversations.py`）零改動，鐵律 1／3 不被觸碰。
- **本地推論收束於新模組 `backend/app/local_embed.py`**（僅 `llm.py` 得 import；CLAUDE.md 鐵律 3 已補註）。懶載入模組級單例 + `asyncio.to_thread`（不阻塞事件迴圈、不拖慢啟動）。
- **只把 `embed_source` 執行期化**（settings_store + `SettingsUpdate` + 守護測試）；embed 端點／key／模型維持 `.env`——來源選擇是使用者層決策（同 `chat_backend` 先例），而 key 執行期化會讓「全庫向量何時失效」不可推理。
- **本地模型常數 `LOCAL_EMBED_MODEL = "BAAI/bge-m3"`**（寫死於 `local_embed.py`；T-EM-00 煙霧測試拍板，決策記錄全文見 `docs/plans/M14-smoke-test-record.md`）：兩候選皆不在 fastembed 0.8.0 原生清單，BGE-M3 以 `add_custom_model` 掛官方 ONNX（CLS pooling + normalize，MIT）；原生替代 multilingual-e5-large 因需前綴、鑑別度差（相似度壓縮 0.80–0.85 vs BGE-M3 的 0.50–0.71）淘汰。**1024 維**對齊現行 `VECTOR(1024)`，零 migration；回傳前 assert 維度 == 1024。實測（24 vCPU/16GB 容器）：模型檔 ~2.2GB（volume 快取）、常駐 RSS ~1.6GB、**必須顯式 `threads=8` + `batch_size=4`**（否則 onnxruntime 吃滿全核峰值 6.6GB）→ 峰值 2.53GB、1.7 passages/s、query 延遲 32ms；中文 query × 真實 chunks 檢索 spot check 數處優於 NIM 基準。fastembed pin `==0.8.0`，image 增量 ~75MB。最低記憶體建議 4GB。
- **本地路徑也要區分 passage/query**（同 D5 的 NIM 陷阱）：BGE-M3 為對稱模型**免前綴**（passage/query 皆直接 encode，此為選它的理由之一）；若日後換模型（如 arctic 需 `"query: "` 前綴），用錯不報錯但檢索品質默默劣化。
- **`llm.effective_embed_config() -> (source, model, dim)` 為單一真相**：backup manifest（`embed_model`／`embed_dim`）、restore 的模型相符判斷、healthz、前端顯示一律共用，反映實際生效來源而非 `.env` 原值。
- **模型檔快取**：config 加 `embed_cache_dir`（`.env` 的 `EMBED_CACHE_DIR`，預設 `/data/models`）+ compose 新 `models:` volume（重建容器不重下載）。requirements 加 `fastembed`（版本由煙霧測試 pin）。**首次使用本地模式需網路下載模型檔（數百 MB）**；下載失敗拋 `LLMError` 走既有 `failed` 狀態機可重試。

#### reembed 維護動作（切換來源後重建向量）

- 混模型向量共存會污染檢索（不同模型的向量空間不可比）。切換 `embed_source` 後以 **`POST /api/maintenance/reembed`**（見 §5，202／409）全庫重嵌：`services/reembed.py` 逐文獻 `get_chunks` → `embed_passages` → `update_chunk_embeddings`（**content 已在 DB，免重解析、免 LLM**）。
- **三方共用鎖**：沿用 backup 的 `try_begin(operation)`／`set_progress` helper——**backup／restore／reembed 三方互斥**（防混模型向量在重嵌途中被 dump），進度走既有 `status` 的 `operation:"reembed"`（逐文獻 `current`/`total`）。
- 切換來源的提示為**前端文案 + 一鍵重建按鈕**（不做全庫模型追蹤——單機工具，追蹤成本大於收益）。

#### B. digest 走 chat_backend 分派

- 新增非串流 **`agent.chat_once(system, user_content, *, max_tokens=3000, deps=None) -> (text, usage)`**：消費 `stream_chat` 事件（累積 token、取 usage），**零複製分派／重試邏輯**（天然繼承 `chat_backend` 分派與降級保險）。`digest.py` 只改呼叫處，`extract_json`／`_validate` 一字不動（鐵律 1 守門）；ThinkFilter 已在 `stream_chat` 內，不雙重過濾。
- **停用工具**：`stream_chat`（`agent` + `claude_backend` 兩份）加 `with_tools: bool = True`，`chat_once` 傳 `False`——**digest 純摘要不需工具**，省 token 且決定性更高；預設 `True` 使既有對話行為零變。
- `max_tokens` 只在 openai 路徑生效（`ModelSettings`）；claude-sdk 忽略（docstring 註明）。
- ~~**`glossary.py` 仍走 `llm.chat`，本次不動**（鐵律 7 範圍紀律）——記入發現事項，留後續小卡處理。~~ **已收尾**（M14 後續，審查修正授權）：glossary 兩個 LLM 呼叫點改走 `agent.chat_once`，翻譯跟著 `chat_backend` 分派，純 Claude 情境零 NIM chat 依賴。

#### C. 備份格式 v2

- `FORMAT_VERSION = 2`；restore 檢查改 **`format_version not in (1, 2)`**——**v1 舊備份繼續可還原**（見 §5 restore 端點與 D11）。
- **chunks dump 每文獻一檔** `chunks/{pdf-uuid}.json`：
  ```
  {embed_model, embed_dim, chunks: [{id, chunk_index, page, section, content, bbox_list, embedding}]}
  ```
  `embedding` 為 **float32 LE → base64**（比 JSON 浮點省 ~60% 體積）。**每次全量覆蓋**（reembed／修復會改向量且無「變更訊號」，正確性優先於增量；manifest `chunk_files` 已備妥未來增量的資訊面）。
- **向量讀回**：新 repo 函式 `dump_chunks(session, doc_id)` 以 `embedding::text` 讀出（與寫入端 `CAST(:emb AS vector)` 對稱）；**不放寬 `_DUMP_TABLE_COLUMNS` 白名單**（chunks 走專用路徑，非 `dump_table_rows`，故 M15 漂移守護不受影響）。
- **manifest v2**：`embed_model`／`embed_dim` 改取 `effective_embed_config()`、`counts` 加 `chunks`、新增 `chunk_files` 清單（見 D10 manifest 結構的 v2 變更註）。

##### 還原三路分派（新文獻）

| 路徑 Path | 條件 Condition | 行為 Behavior |
|---|---|---|
| (a) 直灌 fast | v2 且 manifest 模型 == 現行生效（`effective_embed_config()`） | `insert_chunks` + base64 解碼 `update_chunk_embeddings`——**零 LLM 零解析** |
| (b) 重嵌 re-embed | v2 但模型不符 | `insert_chunks`（content 從 dump）+ `embed_passages`——**免重解析**（僅重算向量） |
| (c) 全重建 full | v1，或 chunk 檔缺失／損壞 | 現行下載 PDF 重跑 `ingest` 路徑（robustness fallback，同 D11 新文獻流程） |

- **chunk 插入提前到 merge phase**（annotations/glossary 的 `chunk_id` remap 需要新 chunk id）；向量填充留 ingest phase（進度沿用 `ingest` phase 名，前端零改動）。狀態機：插 document 時 `status='embedding'`，向量填完轉 `'ready'`。
- **chunk_id remap 規則**：dump 的 annotations/glossary 本就帶舊 `chunk_id`（dump 端零改動）；chunk 檔亦帶舊 `id` → 建 **`old_chunk_id → chunk_index → new_chunk_id`** 映射；`restore_insert_annotation`／`restore_insert_glossary_entry` 加 optional `chunk_id`（**預設 `NULL` ＝ v1 行為**，見 D11——v1 無 chunk 檔可映射，維持 `chunk_id=NULL`）。v2 直灌／重嵌路徑則填回 remap 後的真 `chunk_id`，標註↔chunk 關聯不再丟失。
- **failed 文獻修復路徑同樣升級**（相符即直灌）；冪等收斂保證（D11）在 v2 成立且更快（無需重跑 ingest）。

#### 純 Claude 零 NIM 情境（驗收目標）

`.env` 拿掉 `EMBED_API_KEY` + `chat_backend=claude-sdk`（`embed_source=auto` 自動落本地）→ 上傳／提問／導讀／備份／還原全功能可用，**零 NIM 依賴**。這是 M14 的核心驗收（見 roadmap M14 DoD）。
...
| POST | /api/maintenance/reembed | 全庫向量重建（切換 embed 來源後重嵌）→ 202；backup/restore/reembed 任一進行中回 409 `operation_running`（M14，見 D12） |

### 備份端點詳解（M12 / T-BK-03，見 D10；M13 restore 見 D11）

- `GET /api/backup/status` → `200 {connected: bool, running: bool, operation: "backup"|"restore"|"reembed"|null, progress: {phase, current, total}|null, last_run: {at, ok, error?, counts?}|null, last_restore: {at, ok, error?, summary?}|null, interval_hours: int}`。`connected` 依 `gdrive_refresh_token` 是否存在；`running`/`operation`/`progress` 取自 backup 服務模組級狀態（backup、restore 與 reembed 共用同一把鎖，`operation` 標示當前進行的操作，見 D11／D12）。`progress.phase`：備份為 `pdfs`/`db`/`manifest`；還原為 `download`/`merge`/`ingest`（`ingest` 階段 `current`/`total` 為第 n／共 m 篇）；reembed 逐文獻進度亦以 `current`/`total` 表第 n／共 m 篇（見 D12）。`last_run` 為上次備份摘要、`last_restore` 為上次還原摘要（`summary` 結構見 D11），各自持久化於 `backup_last_run`／`restore_last_run`。
