...
from app.llm import effective_embed_config
...
# =====================================================================
# 向量 codec（M14 D12 C）：pgvector 文字 ⇄ float32 LE base64。
# 隨備份出庫的 chunk 向量以 base64 float32 儲存（比 JSON 浮點省 ~60% 體積）；
# 還原 v2 直灌路徑（T-BK2-02）以 b64_to_vector 解回。放此模組供 backup／restore 共用。
# =====================================================================


def vector_to_b64(vec: str | list[float]) -> str:
    """把向量壓成 float32 LE → base64 字串。

    `vec` 可為 `repo.dump_chunks` 經 `embedding::text` 讀回的 pgvector 字面字串
    （`"[0.1,0.2,...]"`，恰為合法 JSON 陣列），或已解析的 float 清單。
    """
    values = json.loads(vec) if isinstance(vec, str) else vec
    packed = struct.pack(f"<{len(values)}f", *values)
    return base64.b64encode(packed).decode("ascii")


def b64_to_vector(s: str) -> list[float]:
    """把 `vector_to_b64` 的 base64 字串解回 float 清單（float32 LE）。

    防禦：非法 base64（含長度非法、非 base64 字元）或位元組長度非 4 的倍數 → `ValueError`
    （還原端可據此判定 chunk 檔損壞、回落全重建路徑，見 D12 三路分派 (c)）。
    """
    try:
        raw = base64.b64decode(s, validate=True)
    except (binascii.Error, ValueError) as exc:
        raise ValueError(f"非法 base64 向量：{exc}") from exc
    if len(raw) % 4 != 0:
        raise ValueError(f"向量位元組長度 {len(raw)} 非 4 的倍數，無法解為 float32")
    count = len(raw) // 4
...
async def export_chunk_dumps(staging_dir: Path) -> dict[str, int]:
    """匯出每篇有 chunks 的文獻到 `staging_dir/chunks/{pdf-uuid}.json`（M14 D12 C）。

    每檔結構 `{embed_model, embed_dim, chunks: [{id, chunk_index, page, section, content,
    bbox_list, embedding}]}`——`embed_model`／`embed_dim` 取 `effective_embed_config()`
    （反映實際生效來源）；`embedding` 為 float32 LE → base64（`vector_to_b64`），尚未嵌入
    （NULL）的 chunk 照出、`embedding` 欄為 `null`。檔名沿用 PDF 的 UUID（`file_path` stem，
    即 `{uuid}.pdf` → `{uuid}.json`），與 D10 pdfs/ 佈局對齊，供還原以 UUID 對映文獻。

    回傳 `{uuid: chunk 數}` 供 `build_manifest` 組 `chunk_files`。0 chunk 的文獻略過（不寫
    空檔）。向量隨備份出庫→還原可免重嵌免重解析（見 D12 三路分派）。
    """
    chunks_dir = staging_dir / "chunks"
    chunks_dir.mkdir(parents=True, exist_ok=True)

    _, embed_model, embed_dim = effective_embed_config()
    ...
    async with SessionLocal() as session:
        ...
        for doc in documents:
            ...
            if not file_path:
                continue
            rows = await repo.dump_chunks(session, doc["id"])
            if not rows:
                continue
            ...
            chunk_records = [
                {
                    ...
                    "section": r["section"],
                    "content": r["content"],
                    "bbox_list": r["bbox_list"],
                    "embedding": (
                        vector_to_b64(r["embedding"]) if r["embedding"] is not None else None
                    ),
                }
                for r in rows
            ]
            _write_json(
                chunks_dir / f"{uuid}.json",
                {"embed_model": embed_model, "embed_dim": embed_dim, "chunks": chunk_records},
            )
            chunk_counts[uuid] = len(chunk_records)

    return chunk_counts


async def build_manifest(
    counts: dict[str, int], chunk_counts: dict[str, int] | None = None
) -> dict[str, Any]:
    """組 manifest.json 內容（D10 匯出格式，v2 變更見 D12）。

    `counts` 是 `export_db_dumps` 回傳的六表 row 數；`chunk_counts` 是
    `export_chunk_dumps` 回傳的 `{uuid: chunk 數}`（v2）。pdfs 清單另行查 documents 表
    （不假設呼叫者已備妥），取 file_path 的 basename 與實體檔大小。檔案已遺失（本機
    刪除/搬移過)的文獻直接跳過並記 log warning，不中止備份（見 D10 刪除語意）。

    v2：`embed_model`／`embed_dim` 改取 `effective_embed_config()`（反映實際生效來源而非
    `.env`）；`counts` 加 `chunks` 總數；新增頂層 `chunk_files`（每篇一個 chunks/{uuid}.json
    的清單，帶 document_id 供還原對映）。
    """
    _, embed_model, embed_dim = effective_embed_config()
    ...
    for doc in documents:
        ...
        if not file_path:
            continue
        uuid = Path(file_path).stem
        if uuid in chunk_counts:
            chunk_files.append(
                {"name": f"{uuid}.json", "document_id": doc["id"], "chunks": chunk_counts[uuid]}
            )
    ...
    manifest_counts = {**counts, "chunks": sum(chunk_counts.values()), "pdfs": len(pdfs)}
...
async def run_backup() -> None:
    ...
    async with try_begin("backup") as acquired:
        ...
        try:
            ...
            chunks_folder_id = await gdrive.ensure_folder("chunks", parent_id=root_id)
            ...
            chunk_counts = await export_chunk_dumps(staging)

            db_files = sorted((staging / "db").glob("*.json"))
            chunk_files = sorted((staging / "chunks").glob("*.json"))
            remote_db_by_name = {f["name"]: f["id"] for f in await gdrive.list_folder(db_folder_id)}
            remote_chunk_by_name = {
                f["name"]: f["id"] for f in await gdrive.list_folder(chunks_folder_id)
            }

            # chunk 檔與 db dump 同屬「DB 內容全量覆蓋」的一次上傳；D12 未新增 phase 名，
            # 沿用既有 "db" phase 把兩者算進同一 total（前端零改動——體感上仍是「上傳 DB」）。
            # 兩者皆全量覆蓋策略：遠端有同名 update_file，否則 upload_file（同 db/*.json）。
            total_db = len(db_files) + len(chunk_files)
            ...
            for db_file in db_files:
                ...
                uploaded += 1
                set_progress("db", uploaded, total_db)
            for chunk_file in chunk_files:
                content = chunk_file.read_bytes()
                name = chunk_file.name
                if name in remote_chunk_by_name:
                    await gdrive.update_file(
                        remote_chunk_by_name[name], content, "application/json"
                    )
                else:
                    await gdrive.upload_file(chunks_folder_id, name, content, "application/json")
                uploaded += 1
                set_progress("db", uploaded, total_db)

            manifest = await build_manifest(counts, chunk_counts)
